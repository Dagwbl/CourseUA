...
def load_json_relaxed(path):
    """
    Load JSON file but tolerate trailing commas by stripping them before parsing.
    This keeps behavior identical for valid JSON and fixes common formatting mistakes.
    """
...
def example_bridge_analysis():
    ...
    data = load_json_relaxed('bridge_vibration_data.json')
...
def compare_with_known_results():
    ...
    true_mode_data = load_json_relaxed('mode_shapes/intact_mode_data.json')
